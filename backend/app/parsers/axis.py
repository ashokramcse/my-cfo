...
class AxisParser(BaseParser):
    bank_name = "AXIS"
    patterns = [r"AXIS\s+BANK", r"axisbank\.com", r"Axis\s+Credit\s+Card"]
    ...
    def parse(self, text: str) -> ParsedStatement:
        ...
        m = re.search(r"Payment\s+Due\s+Date[:\s]+(\d{2}-[A-Za-z]{3}-\d{4})", text, re.IGNORECASE)
        if m:
            stmt.due_date = self._parse_date(m.group(1))

        stmt.transactions = self._parse_transactions(text)
        stmt.parse_confidence = 0.80 if stmt.transactions else 0.3
        return stmt

    def _parse_transactions(self, text: str) -> list[ParsedTransaction]:
        transactions = []
        pattern = re.compile(
            r"(\d{2}\s+[A-Za-z]{3}\s+\d{4})\s+(.+?)\s+([\d,]+\.\d{2})\s*(Cr)?",
            re.MULTILINE,
        )
