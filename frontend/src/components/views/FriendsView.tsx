...
export function FriendsView() {
  ...
  const activeCount = friends.filter((f) => Number(f.active_emi_count) > 0).length
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {friends.map((friend, i) => {
              ...
              const collectedPct = Number(friend.total_emi_amount) > 0
                ? Math.round((Number(friend.total_collected) / Number(friend.total_emi_amount)) * 100)
                : 0
              return (
                <motion.div
                  key={friend.id}
                ...
                >
                  ...
                  <div>
                    <div className="flex justify-between text-xs text-muted-foreground mb-1.5">
                      <span>{Number(friend.active_emi_count)} active EMIs</span>
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-3 mb-5">
                  {[
                    ...
                    ['Active EMIs', String(Number(selected.active_emi_count))],
                  ].map(([label, value]) => (
