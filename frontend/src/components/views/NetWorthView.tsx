...
function TimelineChart({ data }: { data: HistoryPoint[] }) {
  ...
  const fmt = data.map(d => ({ ...d, label: format(parseISO(d.date), 'dd MMM') }))
  return (
    <ResponsiveContainer width="100%" height={200}>
      <AreaChart data={fmt} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
        ...
        <Tooltip
          ...
          contentStyle={{ background: '#FFFAF7', border: '1px solid #E7E2DC', borderRadius: 12, fontSize: 12 }}
        />
        <Area type="monotone" dataKey="total_assets"      stroke="#10B981" fill="url(#aGrad)" strokeWidth={1.5} dot={false} />
        <Area type="monotone" dataKey="total_liabilities" stroke="#F97316" fill="url(#lGrad)" strokeWidth={1.5} dot={false} />
        <Area type="monotone" dataKey="net_worth"         stroke="#7C3AED" fill="url(#nGrad)" strokeWidth={2.5} dot={false} />
      </AreaChart>
    </ResponsiveContainer>
  )
}
